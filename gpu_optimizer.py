...
def optimize_intermediate_gpu(source_pixels_rgb, target_pixels_rgb, width, height, min_steps=100000, is_auto=True, opt_mode='5d_sliced'):
    ...
    if opt_mode == '3d_rgb_vector':
        ...
        completion_msg = f"RTX 3080 Ti CUDA GPU로 3D RGB 극소값 완전 수렴 완료 (투영 {actual_projections}회 + 2-Opt {actual_batches}회, {time.time()-start_time:.2f}초)"
    else:
        ...
        completion_msg = f"RTX 3080 Ti CUDA GPU 가속으로 5D 시공간 {actual_min_steps:,} 스텝 최적화 완료 ({time.time()-start_time:.2f}초)"
...
def _optimize_cpu_fallback(source_pixels_rgb, target_pixels_rgb, width, height, min_steps):
    ...
    return dest_map.tolist(), {
        ...
        "final_rmse": 0.0,
        "message": f"CPU 폴백으로 최적화 완료 ({elapsed:.2f}초)"
    }
